...
struct StructRedSocial{
    ...
    string toString(){
        return "Red: " + nombre + "\tGusto: " + to_string(prefrerencia) + "\n";
    }
};
...
struct StructPecado{
    ...
    string toString(){
        return "Pecado: " + nombre + "\tCantidad: " + to_string(cantidad) +"\n";
    }
...
struct StructHumano{
    ...
    string toString(){
        ...
        if(vivo)
            ...
            str = "Cielo";
        estado += "ID: " +to_string(id) +"\tNombre: " + nombre +" "  +apellido +"\tEstado: " +str+"\tPais: " +
        pais +"\tCreencia: " +creencia +"\tProfesion: " +profesion+"\tNacimiento: " +nacimiento  +"\nAmigos:\n";
        for (int i = 0; i < amigosEncontrados; i++)
            estado +=  amigos[i]->toString2();
        
        estado += +"\nPecados:\n";
        for (int i = 0; i < 7; i++)
            estado += pecados[i]->toString();
        

        estado += +"\nRedes:\n";
        for (int i = 0; i < 7; i++)
            estado += redesSociales[i]->toString();
    ...
    string toString2(){
        return "\tID: " + to_string(id) + "\tNombre: " +nombre +" " +apellido +"\tPais: " +
        pais +"\tCreencia: " +creencia +"\tProfesion: " +profesion +"\tNacimiento: " +nacimiento +"\n";
    }   
    ...
    void hacerPublicacion1(const string& _red){
        for(int i = 0; i < amigosEncontrados; i++){
            for(int j = 0; j < 7; j++){
                if(amigos[i]->redesSociales[j]->nombre == _red){
